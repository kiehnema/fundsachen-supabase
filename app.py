...
.stButton>button { background-color: #4CAF50; color: white; border-radius: 8px; height: 3em; width: 100%; font-weight: bold; }
.card { background-color: white; padding: 15px; border-radius: 12px; box-shadow: 0px 4px 15px rgba(0,0,0,0.15); margin-bottom: 20px; text-align: center; }
        ...
        with col2:
            ...
            if st.button("📦 Fund speichern"):
                ...
                supabase.storage.from_(BUCKET).upload(
                    file_id, image_bytes, {"content-type": "image/jpeg"}
                )
...
# ------------------------
# TAB 2: SUCHE MIT VORSCHLÄGEN
# ------------------------
with tab2:
    st.subheader("Fundstücke durchsuchen")

    # Mögliche Kategorien für Vorschläge
    categories = ["Flasche", "Pullover", "Schuhe"]

    # Autocomplete über Selectbox
    search_category = st.selectbox(
        "🔍 Kategorie auswählen oder tippen",
        options=[""] + categories,
        index=0
    )

    # Freie Textsuche zusätzlich
    search_text = st.text_input("Zusätzliche Suche (optional)")
    ...
    if st.button("Suchen"):
        ...
        if not show_all:
            if search_category:
                query = query.eq("label", search_category)
            if search_text:
                query = query.ilike("label", f"%{search_text}%")
