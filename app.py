...
.card { background-color: white; padding: 15px; border-radius: 12px; box-shadow: 0px 4px 10px rgba(0,0,0,0.1); margin-bottom: 20px; }
.title { font-size: 40px; font-weight: bold; }
.subtitle { color: grey; margin-bottom: 20px; }
</style>
...
with tab2:
    ...
    # Drei Hauptkategorien nebeneinander
    col_a, col_b, col_c = st.columns(3)
    category_filter = ""
    with col_a:
        if st.button("Schlüssel"):
            category_filter = "Schlüssel"
    with col_b:
        if st.button("Tasche"):
            category_filter = "Tasche"
    with col_c:
        if st.button("Handy"):
            category_filter = "Handy"

    # Textsuche und Alle anzeigen
    ...
    if st.button("Suchen"):
        ...
        if not show_all:
            filters = []
            if category_filter:
                filters.append(("label", category_filter))
            if search_text:
                filters.append(("label_ilike", f"%{search_text}%"))

            # Filter anwenden
            if filters:
                # zuerst Kategorie, dann Text
                if category_filter:
