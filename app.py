...
with tab2:
    ...
    # Drei Kategorien nebeneinander
    col_a, col_b, col_c = st.columns(3)
    category_filter = ""
    with col_a:
        if st.button("Flasche"):
            category_filter = "Flasche"
    with col_b:
        if st.button("Pullover"):
            category_filter = "Pullover"
    with col_c:
        if st.button("Schuhe"):
            category_filter = "Schuhe"

    # Textsuche + Alle anzeigen
    search_text = st.text_input("🔍 Objekt suchen")
    ...
    if st.button("Suchen"):
        ...
        if not show_all:
            if category_filter:
                query = query.eq("label", category_filter)
            if search_text:
